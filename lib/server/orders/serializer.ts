...
import {
  OrderLedgerDataCorruptionError,
  serializePublicEventEvidence,
} from "./public-events";
  ...
  try {
    ...
    if (events.some((event) => event.order_id !== order.id)) {
      throw new OrderLedgerDataCorruptionError(
        "Order event relationship validation failed",
      );
    }
    ...
    return {
      ...
      order: {
        ...
        repair: {
          ...
          patchSpecId: order.patch_spec_id,
          patchSpecSha256: order.patch_spec_sha256,
          patchSpecModelId: order.patch_spec_model_id,
      ...
      events: events.map((event) => {
        const serialized = serializePublicEventEvidence(
          event.event_type,
          event.provider,
          event.payload,
        );
    ...
    if (error instanceof OrderLedgerDataCorruptionError) {
      throw error;
    }
