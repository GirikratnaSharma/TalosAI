...
describe("provider capability configuration", () => {
  ...
  it("reports live readiness using names while keeping every value private", () => {
    const secrets = {
      ...
      pioneerUrl: "https://api.pioneer.ai",
      pioneerKey: "pioneer-secret-should-never-serialize",
      pioneerModel: "fastino/gliner2-large-v1",
    ...
    const environment = parseProviderEnvironment({
      ...
      TALOS_PIONEER_MODE: "live",
      PIONEER_API_URL: secrets.pioneerUrl,
      PIONEER_API_KEY: secrets.pioneerKey,
      PIONEER_MODEL_ID: secrets.pioneerModel,
